...
# 5. Copy ~/.secrets.zsh over (see below), then log out/in.
```
...
`~/.secrets.zsh` is sourced by zsh but lives **outside this repo** — never commit
it. Get it onto a new machine via one of:

```sh
# from an existing machine:
scp ~/.secrets.zsh <vm>:~/.secrets.zsh && ssh <vm> chmod 600 ~/.secrets.zsh

# or store it as a 1Password secure note and:
op read "op://Private/secrets.zsh/notesPlain" > ~/.secrets.zsh && chmod 600 ~/.secrets.zsh
```

Upgrade path: [sops-nix](https://github.com/Mic92/sops-nix) to keep secrets
encrypted in-repo, decrypted per-machine via SSH host keys.
